<template>
  <div class="w-full bg-white shadow-md sticky top-0 z-50">
    <div class="flex justify-between items-center px-4 py-4 md:px-8">
      <NuxtLink to="/" class="text-2xl font-bold text-blue-600 hover:text-blue-800">
        minabevent
      </NuxtLink>
      <button
      ...
      <div class="hidden md:flex items-center space-x-6">
        <NuxtLink to="/user/createEvent" exact class="text-black font-medium px-4 py-2 rounded hover:bg-gray-100 transition">
          Create Event
        </NuxtLink>
        <NuxtLink to="/user/bookMark" exact class="text-black font-medium px-4 py-2 rounded hover:bg-gray-100 transition">
          Book Marks
        </NuxtLink>
        ...
        <div class="relative flex items-center cursor-pointer" @click="hoverClick">
          ...
          <h3 v-else-if="userData" class="text-black font-medium ml-2">
            {{ userData.username }}
          </h3>
      ...
      <div
        ...
        @click="toggleMobileMenu"
      >
        <div class="absolute top-16 right-0 w-full bg-white shadow-lg border-t border-gray-200 flex flex-row p-4">
          <NuxtLink to="/user/createEvent" exact class="text-black font-medium px-4 py-2 rounded hover:bg-gray-100 transition">
            Create Event
          </NuxtLink>
          <NuxtLink to="/user/bookMark" exact class="text-black font-medium px-4 py-2 rounded hover:bg-gray-100 transition">
